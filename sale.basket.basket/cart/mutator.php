...
foreach ($this->basketItems as $row)
{
    ...
    $rid = 0;
    $newProps = [];
    foreach($row['PROPS'] as $k=>$prop){
        if($prop['CODE'] == 'RID'){
            $rid = $prop['VALUE'];
            continue;
        }
        if($prop['CODE'] == 'PARENT_ID'){
            $parentProd = $prop['VALUE'];
            continue;
        }
        $newProps[] = $prop;
    }
    $row['PROPS'] = $newProps;
    //echo'<pre>';print_r($row['PROPS']);echo'</pre>';
    if($parentProd){
        $r = CIblockElement::getById($parentProd)->getNextElement();
        $fields = $r->getFields();
        $pictId = 0;
        $prodUrl = '';
        if(!$fields['PREVIEW_PICTURE']){
            $props = $r->getProperties();
            if(isset($props['CML2_LINK']['VALUE'])){
                $r2 = CIblockElement::getById($props['CML2_LINK']['VALUE'])->getNextElement();
                $fields2 = $r2->getFields();
                if($fields2['PREVIEW_PICTURE']){
                    $pictId = $fields2['PREVIEW_PICTURE'];
                }
                if($fields2['DETAIL_PAGE_URL']){
                    $prodUrl = $fields2['DETAIL_PAGE_URL'];
                }else{
                    $prodUrl = $fields['DETAIL_PAGE_URL'];
                }
            }else{
                $prodUrl = $fields['DETAIL_PAGE_URL'];
            }
        }else{
            $pictId = $fields['PREVIEW_PICTURE'];
            $prodUrl = $fields['DETAIL_PAGE_URL'];
        }
        if($pictId){
            $row['PREVIEW_PICTURE_SRC'] = CFile::getPath($pictId);
        }
        $row['DETAIL_PAGE_URL'] = $prodUrl;
        $row['DETAIL_PAGE_URL'] .= '?rid='.$rid;
    }
