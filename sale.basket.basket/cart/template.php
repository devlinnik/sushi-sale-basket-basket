...
if (empty($arResult['ERROR_MESSAGE']))
{
    ...
    <script>
        (function () {
            ...
            function getBasketItem(node) {
                return node ? node.closest('[data-entity="basket-item"]') : null;
            }

            function getQtyInput(item) {
                return item ? item.querySelector('[data-entity="basket-item-quantity-field"]') : null;
            }

            function getMinusButton(item) {
                return item ? item.querySelector('[data-entity="basket-item-quantity-minus"]') : null;
            }

            function updateMinusState(item, qty) {
                var minusBtn = getMinusButton(item);
                if (!minusBtn) {
                    return;
                }

                if (qty <= 1) {
                    minusBtn.classList.add('product-item-amount-field-btn-disabled');
                } else {
                    minusBtn.classList.remove('product-item-amount-field-btn-disabled');
                }
            }
            ...
            function bindBasketQuantityHandlers() {
                ...
                document.addEventListener('change', function (e) {
                    ...
                    applyQty(input, input.value, false);
                });
            ...
            function syncAllMinusButtons() {
                var items = document.querySelectorAll('[data-entity="basket-item"]');
