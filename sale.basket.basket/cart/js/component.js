;(function() {
	...
	BX.Sale.BasketComponent = {
		...
		bindQuantityEvents: function(node, data)
		{
			...
			var minusBtn = this.getEntity(block, 'basket-item-quantity-minus');
			var plusBtn = this.getEntity(block, 'basket-item-quantity-plus');
			var field = this.getEntity(block, 'basket-item-quantity-field');

			if (minusBtn)
			{
				BX.bind(minusBtn, 'click', BX.delegate(function(e) {
					e.preventDefault();

					var itemData = this.getItemDataByTarget(minusBtn);
					if (!itemData)
						return;
					...
					var currentQuantity = isQuantityFloat ? parseFloat(quantityField.value) : parseInt(quantityField.value, 10);
					var step = this.getQuantityStep(itemData);

					currentQuantity = currentQuantity || 0;
					step = step || 1;

					var newQuantity = currentQuantity - step;
					newQuantity = this.getCorrectQuantity(itemData, newQuantity);

					quantityField.value = newQuantity;

					BX.fireEvent(quantityField, 'change');
				}, this));
			}

			if (plusBtn)
			{
				BX.bind(plusBtn, 'click', BX.delegate(function(e) {
					e.preventDefault();

					var itemData = this.getItemDataByTarget(plusBtn);
					if (!itemData)
						return;
					...
					console.log('PLUS', {
						...
						measureRatio: itemData.MEASURE_RATIO
					});
			...
			if (field)
			{
				BX.bind(field, 'change', BX.proxy(this.quantityChange, this));
			}
		},

		startQuantityInterval: function(event, target)
		{
			...
			var func = target.getAttribute('data-entity') === 'basket-item-quantity-minus'
				? BX.delegate(function(){ this.quantityMinus(target); }, this)
				: BX.delegate(function(){ this.quantityPlus(target); }, this);
		...
		{
			console.log('quantityPlus target:', target);
			console.log('proxy_context:', BX.proxy_context);
			...
			if (itemData)
			{
				...
				var quantity = parseFloat((currentQuantity + step).toFixed(5));
		...
		getQuantityStep: function(itemData)
		{
			var isQuantityFloat = this.isQuantityFloat(itemData);

			// Для дробного количества оставляем стандартный шаг
			if (isQuantityFloat)
			{
				return parseFloat(itemData.MEASURE_RATIO) || 1;
			}

			// Для обычных товаров всегда шаг 1
			return 1;
		},
		...
		getCorrectQuantity: function(itemData, quantity)
		{
			...
			if (itemData.CHECK_MAX_QUANTITY === 'Y')
			{
				availableQuantity = isQuantityFloat
					...
					: parseInt(itemData.AVAILABLE_QUANTITY, 10);
			...
			// Для целых товаров шаг всегда 1
			if (!isQuantityFloat)
			{
				return parseInt(quantity, 10);
			}

			// Для дробных товаров оставляем логику кратности MEASURE_RATIO
			var measureRatio = parseFloat(itemData.MEASURE_RATIO) || 1;
